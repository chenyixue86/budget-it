...
export default function VasteLastenPage() {
  ...
  const [categorie, setCategorie] = useState<string>(t.vasteLasten.categories[6]);
  ...
  async function add(e: React.FormEvent) {
    ...
    if (err) {
      ...
      setNaam("");
      setBedrag("");
      setCategorie(t.vasteLasten.categories[6]);
      load();
    }
    setSaving(false);
  }
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t.vasteLasten.title}</h1>
        <p className="text-gray-600 dark:text-white/60 mt-1 text-sm">{t.vasteLasten.subtitle}</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#111111] rounded-2xl border border-gray-100 dark:border-white/10 p-6 mb-6 transition-colors duration-200">
        <p className="text-sm text-gray-600 dark:text-white/60 mb-1">{t.vasteLasten.totaalPerMaand}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="bg-white dark:bg-[#111111] rounded-2xl border border-gray-100 dark:border-white/10 p-6 transition-colors duration-200">
          ...
          <form onSubmit={add} className="space-y-3">
            ...
            >
              {saving ? t.vasteLasten.opslaan : t.vasteLasten.toevoegenBtn}
            </button>
          </form>
        </div>
